...
func get_push_dir():
  ...
  # var push_angle = pushing_player.global_position.angle_to_point(center.global_position)
  # var dir_i = int((push_angle / TAU + 0.5) * 4 + 0.5) % 4 # im sorry
  
  # print(dir_i)
  # return dir_i if dirs_held[dir_i] else null
  
  # there used to be some complicated logic here but it got fiddly around edges and honestly this
  # works well enough
